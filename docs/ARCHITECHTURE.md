...
AI models must NOT modify the following system boundaries:

Scanner → only enumerates resources 
Graph Engine → builds infrastructure graph 
Detection Engine → performs deterministic security detection 
AI Layer → explains findings and generates remediation advice

Detection logic must never rely on AI.

Yes — and this is actually a **very good idea**.

A strong `ARCHITECTURE.md` dramatically improves how coding agents (Claude Code, Qwen Code, Cursor, etc.) understand your system. Without it they **hallucinate structure**, which causes bad code suggestions.

Since your project is essentially a **Cloud Attack Path Analysis System**, the architecture doc should describe:
...
Below is a **clean, production-grade `ARCHITECTURE.md` written in markdown prompt style** that works extremely well for AI coding assistants.
